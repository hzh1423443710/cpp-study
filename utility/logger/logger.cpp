...
void Logger::open(const std::string& path) {
	...
	if (m_writer.fail()) {
		throw std::runtime_error(std::to_string(__LINE__) +
								 ": Exception: open log file failed, filename:" + m_filename);
	}
	m_filename = path;
...
bool Logger::log(Level level, const char* file, int line, const char* format, ...) {
	...
	if (!m_writer.is_open()) {
		throw std::runtime_error(std::to_string(__LINE__) + ": Exception: Unopened file");
	}
...
void Logger::rotate() {
	...
	if (0 != rename(m_filename.c_str(), newname.c_str())) {
		throw std::runtime_error(std::to_string(__LINE__) + ": Exception: rename log failed" +
